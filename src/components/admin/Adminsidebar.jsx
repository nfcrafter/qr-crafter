...
export default function AdminSidebar({ onFolderSelect, selectedFolder }) {
    ...
    async function deleteFolder(id, e) {
        e.stopPropagation()
        const ok = window.confirm('Supprimer ce dossier ? Les cartes ne seront pas supprimées.')
        if (!ok) return
    ...
    const NAV_ITEMS = [
        { label: 'Dashboard', icon: '📊', path: '/admin', id: 'dashboard' },
        { label: 'Toutes les cartes', icon: '💳', path: '/admin', filter: null, id: 'all' },
        { label: 'Actives', icon: '✅', path: '/admin', filter: 'active', id: 'active' },
        { label: 'En attente', icon: '⏳', path: '/admin', filter: 'pending', id: 'pending' },
    ]
    ...
    return (
        <aside style={{
            ...
            <div style={{ padding: '12px 8px' }}>
                ...
                {[
                    { label: 'Toutes les cartes', icon: '💳', filter: 'all' },
                    { label: 'Actives', icon: '✅', filter: 'active' },
                    { label: 'En attente', icon: '⏳', filter: 'pending' },
                ].map(item => {
                    const isActive = !selectedFolder && (
                        item.filter === 'all'
                            ? location.pathname === '/admin'
                            : false
                    )
                    return (
                        <button key={item.filter} onClick={() => {
                            navigate('/admin')
                            onFolderSelect(null, item.filter === 'all' ? null : item.filter)
                        }} style={{
                        ...
                        >
                            ...
                            {item.filter === 'all' && <span style={{ fontSize: '11px', background: 'var(--accent-light)', color: 'var(--accent)', padding: '1px 6px', borderRadius: '10px', fontWeight: '600' }}>{stats.total}</span>}
                        </button>
                    )
                })}
            </div>
            ...
            <div style={{ padding: '0 8px', flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 8px 4px' }}>
                    ...
                    <button onClick={() => setShowNewFolder(s => !s)} style={{
                        ...
                        fontSize: '16px', color: 'var(--accent)', padding: '2px',
                    ...
                    <div style={{ padding: '10px', background: 'var(--bg)', borderRadius: 'var(--radius-md)', marginBottom: '8px', border: '1px solid var(--border)' }}>
                        ...
                        <div style={{ marginBottom: '8px' }}>
                            <select value={newFolderType} onChange={e => setNewFolderType(e.target.value)}
                                ...
                                <option value="custom">📁 Dossier libre</option>
                ...
                {/* Liste dossiers */}
                {folders.map(folder => (
                    <button key={folder.id} onClick={() => onFolderSelect(folder.id, null)} style={{
                        width: '100%', display: 'flex', alignItems: 'center', gap: '8px',
                        padding: '9px 10px', borderRadius: 'var(--radius-md)',
                        border: 'none', cursor: 'pointer', fontSize: '13px',
                        fontFamily: 'var(--font-body)', fontWeight: '500',
                    ...
                    >
                        ...
                        <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {folder.type === 'city' ? '🏙️ ' : folder.type === 'status' ? '📊 ' : '📁 '}
                            {folder.name}
                        </span>
                        <button className="del-btn" onClick={e => deleteFolder(folder.id, e)} style={{
                            background: 'none', border: 'none', cursor: 'pointer',
