export default function PhoneForm({ content, onChange }) {
    return (
        <div className="card">
            ...
            <div className="field">
                ...
                <input type="tel" placeholder="+33 6 12 34 56 78"
                    value={content.phone || ''}
                    onChange={e => onChange({ ...content, phone: e.target.value, name: content.name || e.target.value })}
                />
            </div>
            <div className="field">
                <label>Nom du code QR</label>
                <input type="text" placeholder="Par exemple : Mon téléphone"
                    value={content.name || ''}
