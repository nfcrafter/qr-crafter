...
export default function PublicProfile() {
  ...
  async function loadProfile() {
    ...
    // Use type_data (admin-filled profile) — admin_profile as fallback key
    const adminData = card.type_data || card.admin_profile || {}
    setProfile(adminData)

    // Custom links from type_data
    setCustomLinks(adminData.customLinks || [])
  ...
  function saveContact() {
    ...
    if (profile.job_title || profile.title) vc += `TITLE:${profile.job_title || profile.title}\r\n`
    ...
    // Socials from type_data.socials
    const socials = profile.socials || {}
    SOCIAL_LINKS.forEach(s => {
      const val = socials[s.key] || profile[s.key]
      if (val) vc += `URL;TYPE=${s.label}:${s.getUrl(val)}\r\n`
    })
  ...
  const themeColor = profile?.primaryColor || profile?.theme_color || '#1A1265'

  // Build active links from socials object + direct profile keys
  const socials = profile?.socials || {}
  const activeLinks = SOCIAL_LINKS.filter(s => {
    const val = socials[s.key] || profile?.[s.key]
    return val && val.trim() !== ''
  })
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f2f5', fontFamily: "'Inter', 'Outfit', sans-serif" }}>
      <style>{`
        ...
        .profile-link { transition: transform .18s, box-shadow .18s; }
        .profile-link:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(0,0,0,0.10) !important; }
      ...
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '0 16px 48px' }}>
        ...
        {/* Profile card */}
        <div style={{
          background: 'white', borderRadius: 20, marginTop: -24, marginBottom: 14,
          boxShadow: '0 4px 28px rgba(0,0,0,0.09)', animation: 'fadeUp 0.4s ease',
          overflow: 'visible', position: 'relative'
        }}>
          {/* Avatar */}
          <div style={{ padding: '0 20px', position: 'relative', height: 52 }}>
            <div style={{
          ...
          <div style={{ padding: '4px 20px 16px' }}>
            ...
            {(profile?.job_title || profile?.title) && (
              <p style={{ fontSize: 13, color: themeColor, fontWeight: 700, marginBottom: 6 }}>{profile.job_title || profile.title}</p>
            )}
            {profile?.bio && (
              <p style={{ fontSize: 14, color: '#555', lineHeight: 1.65, marginBottom: 4 }}>{profile.bio}</p>
            )}
          </div>

          {/* Save contact button */}
          <div style={{ padding: '0 16px 20px' }}>
            <button className="save-btn" onClick={saveContact} style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, animation: 'fadeUp 0.5s ease' }}>
            ...
            {activeCustomLinks.map((link, i) => (
              <a key={i}
                ...
                <div style={{
                  width: 44, height: 44, borderRadius: 12, flexShrink: 0,
                  background: '#F0F4FF', fontSize: 22,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>{link.emoji || '🔗'}</div>
                <span style={{ fontWeight: 700, fontSize: 15, flex: 1 }}>{link.label || 'Lien'}</span>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#bbb" strokeWidth="2.5"><polyline points="9 18 15 12 9 6"/></svg>
              </a>
            ))}
          </div>
